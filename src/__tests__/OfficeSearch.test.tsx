...
describe('OfficeSearch', () => {
    ...
    test('gir riktig respons ved søk på postnummer for postbokser', async () => {
        ...
        searchForText('0614');
        await waitFor(() => {
            expect(screen.getByText('0614 er et postnummer for postbokser i', {exact: false})).toBeInTheDocument();
            expect(screen.getByText('OSLO')).toBeInTheDocument();
            expect(screen.getByText('kommune. Kommunens NAV-kontorer:', {exact: false})).toBeInTheDocument();
    ...
    test('gir riktig respons ved søk på servicepostnummer', async () => {
        ...
        searchForText('0614');
        await waitFor(() => {
            expect(screen.getByText('0614 er et servicepostnummer i', {exact: false})).toBeInTheDocument();
            expect(screen.getByText('OSLO')).toBeInTheDocument();
            expect(screen.getByText('kommune. Kommunens NAV-kontorer:', {exact: false})).toBeInTheDocument();
            expect(getLinkByName('NAV Alna')).toBeInTheDocument();
            expect(getLinkByName('NAV Bjerke')).toBeInTheDocument();
    ...
    test('gir riktig respons ved søk på postnummer uten spesifikt tilknyttet kontor', async () => {
        ...
        searchForText('0354');
        await waitFor(() => {
            expect(screen.getByText('Fant ingen kontor spesifikt tilknyttet', {exact: false})).toBeInTheDocument();
            expect(screen.getByText('0354')).toBeInTheDocument();
            expect(screen.getByText('Alle kommunens NAV-kontorer:', {exact: false})).toBeInTheDocument();
            expect(getLinkByName('NAV Alna')).toBeInTheDocument();
            expect(getLinkByName('NAV Bjerke')).toBeInTheDocument();
            expect(getLinkByName('NAV Frogner')).toBeInTheDocument();
        });
